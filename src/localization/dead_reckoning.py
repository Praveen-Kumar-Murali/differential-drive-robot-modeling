...
class DR_3DOFDifferentialDrive(Localization):
    ...
    def Localize(self, xk_1, uk):  # motion model
        ...
        # Decode wheel increments from pulses: Step 1
        n_L, n_R = uk.flatten()

        # Convert pulses to wheel displacements
        # s = (2πR / pulses_per_turn) * n
        delta_sL = (2 * np.pi * self.wheelRadius / self.robot.pulse_x_wheelTurns) * n_L
        delta_sR = (2 * np.pi * self.wheelRadius / self.robot.pulse_x_wheelTurns) * n_R

        # Compute linear and angular displacement: Step 2 
        delta_s = (delta_sR + delta_sL) / 2.0
        delta_theta = (delta_sR - delta_sL) / self.wheelBase

        # Update pose: Step 3
        theta_prev = xk_1[2, 0]
        x_new = xk_1[0, 0] + delta_s * np.cos(theta_prev + delta_theta / 2.0)
        y_new = xk_1[1, 0] + delta_s * np.sin(theta_prev + delta_theta / 2.0)
        ...
        xk = np.array([[x_new], [y_new], [theta_new]])
        self.xk = xk
        self.Log(self.robot.xsk, self.xk)
    ...
    def GetInput(self):
        ...
        # Read simulated encoders
        zsk, Rsk = self.robot.ReadEncoders()

        # Store them as input vector uk
        uk = zsk
        self.Rsk = Rsk
