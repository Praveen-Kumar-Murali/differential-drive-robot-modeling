...
class Pose3D(np.ndarray):
    ...
    def oplus(AxB, BxC):
        ...
        xC = xA + xB * np.cos(thetaA) - yB * np.sin(thetaA)
        yC = yA + xB * np.sin(thetaA) + yB * np.cos(thetaA)
        thetaC = thetaA + thetaB

        return Pose3D(np.array([[xC], [yC], [thetaC]]))
    ...
    def ominus(AxB):
        ...
        xB = -xA * np.cos(thetaA) - yA * np.sin(thetaA)
        yB =  xA * np.sin(thetaA) - yA * np.cos(thetaA)
        thetaB = -thetaA

        return Pose3D(np.array([[xB], [yB],[thetaB]]))
